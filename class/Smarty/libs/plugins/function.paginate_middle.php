 ...
 * @version 1.5
 *
 * Modified by Wagon Trader
 * for mod_rewrite compatibility
 * rewrite = title to display in url
 * cat = category id
 * list = category page when displaying a list
 */
...
function smarty_function_paginate_middle($params, &$smarty) {
    ...
    while($_item <= $_total) {
        ...
        if($_item != $_curr_item) {
			...
			if( $cat_page != "" ){
				$_this_href = ($rewrite) ? '<a href="' . $mod_title . '-'. $cat .'-'. $cat_page .'-'. $_item .'.html"' . $_attrs . '>' : '<a href="' . str_replace('&','&amp;', $_url) . '"' . $_attrs . '>';
			}else{
				$_this_href = ($rewrite) ? '<a href="' . $mod_title . '-'. $cat .'-'. $_item .'.html"' . $_attrs . '>' : '<a href="' . str_replace('&', '&amp;', $_this_url) . '"' . $_attrs . '>';
			}
            $_ret .= $_link_prefix . $_this_href . $_text . '</a>' . $_link_suffix;
        } else {
