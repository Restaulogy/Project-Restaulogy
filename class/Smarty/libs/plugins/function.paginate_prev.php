...
function smarty_function_paginate_prev($params, &$smarty) {
    ...
    if(($_item = SmartyPaginate::_getPrevPageItem($_id)) !== false) {
		...
		if( $cat_page != "" ){
			$_href = ($rewrite) ? '<a href="' . $mod_title . '-'. $cat .'-'. $cat_page .'-'. $_item .'.html"' . $_attrs . '>' : '<a href="' . str_replace('&','&amp;', $_url) . '"' . $_attrs . '>';
		}else{
			$_href = ($rewrite) ? '<a href="' . $mod_title . '-'. $cat .'-'. $_item .'.html"' . $_attrs . '>' : '<a href="' . str_replace('&','&amp;', $_url) . '"' . $_attrs . '>';
		}
    } else {
        $_show = false;   
    }
    
    return $_show ? $_href . $_text . '</a>' : '';
}
