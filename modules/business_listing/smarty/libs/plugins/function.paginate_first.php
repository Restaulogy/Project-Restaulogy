...
function smarty_function_paginate_first($params, &$smarty) {
	...
	if( $cat_page != "" ){
		$_href = ($rewrite) ? '<a href="' . $mod_title . '-'. $cat .'-'. $cat_page .'-1.html"' . $_attrs . '>' : '<a href="' . str_replace('&','&amp;', $_url) . '"' . $_attrs . '>';
	}else{
		$_href = ($rewrite) ? '<a href="' . $mod_title . '-'. $cat .'-1.html"' . $_attrs . '>' : '<a href="' . str_replace('&','&amp;', $_url) . '"' . $_attrs . '>';
	}    
