...
{/if}
    ...
    <!--<a href="#promo_dialog{$user_promotion[pitm].id}" data-rel="dialog">-->
        ...
        <table style='margin:0px;font-size:10px;width:100%;position:relative;'>
            ...
            <tr>
                <td style="width:65%;vertical-align:top !important;">{$user_promotion[pitm].start_date|date_format:"%D"}&nbsp;-&nbsp;{$user_promotion[pitm].end_date|date_format:"%D"}</td>
            </tr>
            <tr>
                <td style="width:65%;vertical-align:top !important;">#Views :{$user_promotion[pitm].views_count}&nbsp;<b style="color:{$site_color.orange}">{if $user_promotion[pitm].cupon_type  eq 'all_site'}All Site Coupon{elseif $user_promotion[pitm].cupon_type  eq 'recommendation'}Recommendations Coupon{elseif $user_promotion[pitm].cupon_type  eq 'survey'}Survey Coupon{else}{/if}</b></td>
            </tr>
            <tr>
		...
		addthis:description="{$user_promotion[pitm].title}">
           ...
           {if $elgg_current_user}
                 {if $user_promotion[pitm].cupon_type eq 'none'}
                    {if $user_promotion[pitm].is_promo_fav eq 0}
                			<a href="#" onclick='document.location.href="favorite.php?new=1&show_type=PR&lid={$user_promotion[pitm].id}&uid={$vs_current_user.id}"'       style="display:inline-block;" ><img src='{$elgg_small_icon_url}bookmark_add.png'/>Save Promotion</a>
                	{else}
                  ...
                  {if $user_promotion[pitm].cupon_type neq 'reward'}
                    {if $user_promotion[pitm].coupon.id gt 0}
                       {if $user_promotion[pitm].coupon.is_redimed eq 1}
                               Already Redeemed
                       {else}
                         <a href="coupon.php?redim_id={$user_promotion[pitm].coupon.id}&user_redimed=1"><img src='{$elgg_small_icon_url}bookmark_remove.png'/>Redeem Coupon</a>
                       {/if}
            		{else}
                         {if $user_promotion[pitm].cupon_type eq 'survey' && $user_promotion[pitm].is_surveyed neq 1}
                        		<a href="{$elgg_main_url}pg/nabopoll/main/?id_txtserch={$list[itm].userid}" target="_blank" title="Survey"><img src="{$elgg_small_icon_url}add.png" alt="Survey"/>To get coupon please take survey by business.</a>
                    	 {elseif $user_promotion[pitm].cupon_type eq 'recommendation' && $user_promotion[pitm].is_recommended neq 1}
                         		<a href="#" onclick="PopupCenter('{$elgg_main_url}facebook/recommendation.php?isPopup=1&amp;type=1&amp;bizness_id={$list[itm].id}&amp;is_in_cupobiz=1&amp;biz_text={$list[itm].firm}','',425,350);" title="Recommend"><img src="{$elgg_small_icon_url}add.png" alt="Recommend"/>To get coupon please recommend the business.</a>
                    	 {else}
                     			<a href="coupon.php?save=1&promotion_id={$user_promotion[pitm].id}&user_id={$elgg_current_user}"><img src='{$elgg_small_icon_url}bookmark_add.png'/>Save Coupon</a>
                    	 {/if}
            	    {/if}
                  {/if}
                 {/if}
