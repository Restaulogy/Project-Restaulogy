...
<fieldset style="border: 1px solid #ccc;margin:5px;padding:5px;">
    <legend><label> Basic Details</label></legend>
			...
			<td class="detail_left_td">
                ...
                {if ($promotion.id && $is_renew_promotion neq 1) OR ($smarty.request.is_exclusive eq 1)}
                    ...
                    {elseif $promotion.cupon_type eq 'invitation'}
                        Invite only
                    {elseif $promotion.cupon_type eq 'refer_friend'}
                        Refer a friend    
                ...
                {else}
                    <select name="cupon_type" id="cupon_type" onchange="toggle_allow_multi_redeem(this.value);">
                       ...
                       <option value="refer_friend" {if $promotion.cupon_type eq 'refer_friend'}selected{else} {if $coupon_type eq "refer_friend"}selected{else}{/if}{/if}>Refer a friend</option>
                    </select>
                {/if}
                <!--
		...
		<tr id="prm_refer_frd_points_row">
             <td class="detail_right_td">Award points</td>
			<td class="detail_left_td">
                   <input type="text" name="prm_refer_frd_points" id="prm_refer_frd_points" {if $promotion.prm_refer_frd_points neq ''} value="{$promotion.prm_refer_frd_points}"{else} value='0' {/if}/>
			</td>
         </tr>
        <tr><td colspan="2"><span class="error"  id="prm_refer_frd_points_error"></span></td></tr>
...
//..For firm
    ...
    */
	...
	$("#prm_refer_frd_points_error").html("");	
...
if ($('#chk_cond_daytime').is(':checked')) {
	...
	if((elemById("cupon_type").value =='refer_friend')){
		if(IsNonEmpty(elemById("prm_refer_frd_points").value)==false){
			$("#prm_refer_frd_points_error").html("Please Enter Award point.");
			validform = false;
		}       	
    }
...
{literal}
		...
		/*$("#promotion_form :input").change(function() {
		   $("#promotion_form").data("changed",true);
		});*/	
	
    });
    
  /*  function check_if_data_saved(var_redirect){
    	if ($("#promotion_form").data("changed")) {
		  if (confirm("Please save the changes before redirection") == true) {
		  	window.location.href=var_redirect;
		  }
		}
    }*/
