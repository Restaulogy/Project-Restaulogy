...
*Fill Your Filter Criteria To Find Your Type Of {if $isPromotions == 1}Promotions{else}Business Listing{/if}
    ...
    <table width=100% >
         ...
         <script type="text/javascript">
			...
			{
                   $( "#dialog_intrested_category" ).dialog({
         ...
         {/literal}
         <a href="#" onclick='$( "#dialog_intrested_category" ).dialog("open");'>Select Category</a>
         <div id="dialog_intrested_category">
...
<input type = "text" name="categories" id="my_categories" style="display:none;"  class="required"  value ="{if $categories_str}{$categories_str}{/if}"   />
 ...
 <script type="text/javascript">
  $('#intrested_states').change(function(){
      var state_val = $('#intrested_states').val();

        if (state_val > 0){

		document.getElementById('intrested_metro_area_box').innerHTML= '<select Disabled><option>Select Metro Area</option></select>';
		$.post("{/literal}{$website}{literal}/ajax/get_chid_categories.php",
            {parent_id: state_val
            },
              function(response){

                document.getElementById('intrested_metro_area_box').innerHTML= '<select id="intrested_metro_area"  name="metro_area">'+  unescape(response)+ '</select>';
            }
        );
		return false;
        }else{
