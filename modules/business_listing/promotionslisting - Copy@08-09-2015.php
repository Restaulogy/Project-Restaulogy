...
if ($show_type == 'PR'){
    ...
    //$promtionsql = " AND pds_list_promotions.end_date>=CURDATE() AND Find_in_set('".$vs_user_prof_loc['metroareaid']."', pds_list_promotions.metro_area) AND cupon_type LIKE 'none' ";
		...
		*
		...
		$promtionsql = " AND pds_list_promotions.`prm_restaurent`=".$_SESSION[SES_RESTAURANT]." AND pds_list_promotions.end_date>=CURDATE()";
...
switch ($listing_type){
	...
	case "favorite":
		    if($ispromotion == 1){
                $sql =  "SELECT SQL_CALC_FOUND_ROWS {$record_fetch_condition} FROM pds_list_promotions  left outer join pds_list  on pds_list_promotions.list_id = pds_list.id inner join pds_list_favorites  on pds_list_promotions.id = pds_list_favorites.list_id WHERE pds_list_favorites.ispromotion = 1 and pds_list_promotions.end_date>=CURDATE() and pds_list_favorites.user_id=".(is_gt_zero_num($_SESSION['guid'])?$_SESSION['guid']:0)." ORDER BY {$sort_on} {$sort_by} LIMIT $xlimit,$ylimit;";
   ...
   case "post":
            ...
            $sql = "SELECT SQL_CALC_FOUND_ROWS  DISTINCT pds_list.* , pds_list_promotions.id as tmp_promo_id  FROM pds_list_promotions inner join pds_list   on pds_list_promotions.list_id = pds_list.id WHERE pds_list_promotions.end_date>=CURDATE() AND pds_list.userid='".(is_gt_zero_num($_SESSION['guid'])?$_SESSION['guid']:0)."' ORDER BY pds_list_promotions.id desc LIMIT $xlimit,$ylimit;";
	...
	case "expired":
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS  DISTINCT pds_list.* , pds_list_promotions.id as tmp_promo_id  FROM pds_list_promotions inner join pds_list   on pds_list_promotions.list_id = pds_list.id WHERE pds_list_promotions.prm_restaurent='".(is_gt_zero_num($_SESSION[SES_RESTAURANT])?$_SESSION[SES_RESTAURANT]:0)."' AND pds_list_promotions.end_date < CURDATE()  ORDER BY {$sort_on} {$sort_by} LIMIT $xlimit,$ylimit;";
		$promotion_sql_filter = " AND pds_list_promotions.end_date < CURDATE() ";
			$bread_crumb[0] =  "Expired Promotions";
			break;	
	 case "coupon":
			$sql = "SELECT SQL_CALC_FOUND_ROWS DISTINCT pds_list.* , pds_list_promotions.id as tmp_promo_id FROM pds_list inner join pds_list_promotions on pds_list.id = pds_list_promotions.list_id inner join pds_listcat on pds_listcat.list_id = pds_list.id inner join pds_category on pds_category.id = pds_listcat.cat_id WHERE state='apr' AND pds_list_promotions.end_date>=CURDATE() AND pds_list.metro_area =".$vs_user_prof_loc['metroareaid']." AND  cupon_type not in ('none','reward') ORDER BY pds_list_promotions.id DESC, firm LIMIT  $xlimit,$ylimit;";
			//$ispromotion = 2;
