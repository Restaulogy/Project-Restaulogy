	...
	if ($promotion_count > 0)
	    {
			...
			SELECT DISTINCT
				...
				pds_list_promotions.prm_allow_multi_redeem,pds_list_promotions.prm_restaurent,
			...
			if ($result){
                for($y=0;$y<mysql_num_rows($result);$y++){
					...
					$list[$x]['user_promotion'][$y]['formatted_end_date'] =date('m/d/Y',strtotime($list[$x]['user_promotion'][$y]['end_date']));
					$list[$x]['user_promotion'][$y]['formatted_start_date'] =date('m/d/Y',strtotime($list[$x]['user_promotion'][$y]['start_date']));
...
//Get viewed count
function GetBusPromViwedCount($buss_id=0){
	if(is_gt_zero_num($buss_id)){
		$sql ="SELECT COUNT(`hits_id`) as `biz_count` FROM `pds_promotion_hits` WHERE `business_id`={$buss_id}";
		$result =mysql_query($sql);
	}    
    if($result){
...
function getAvailPromForOrder($order_id,$user_id){
	$rows = array();
	if(is_gt_zero_num($order_id)){
		...
		$app_rewards=members::chk_all_rewards($user_id);
		//print_r($app_rewards);
		foreach($app_rewards as $_ech_rwd){
			if($_ech_rwd['can_claim']>0){					
			}else{
				$_promotions[]=$_ech_rwd[RWD_COUPON_ID];
			}			
		}
		if(is_not_empty($_promotions)){
			$_rwd_cond=" AND `pds_list_promotions`.`id` NOT IN (". implode(',',$_promotions) .") ";
		}
		//..now check if that promotion is sign up promotion
		$sql="SELECT `promotion_id` FROM `pds_redim_cupons` WHERE `order_id`={$order_id} AND `biz_redimed`=1";
	    $_complt_bill_prom=DB::ExecScalarQry($sql);
	    if(is_gt_zero_num($_complt_bill_prom)==false){
			//`cupon_type`="all_site" AND 
			$_sql='SELECT `pds_list_promotions`.`id` , `pds_list_promotions`.`list_id` , `pds_list_promotions`.`title`, `pds_list_promotions`.`end_date` FROM `pds_list_promotions` 
					WHERE pds_list_promotions.`prm_restaurent`='.$_SESSION[SES_RESTAURANT].' AND `cupon_type`!="refer_friend" 
					AND `pds_list_promotions`.`id` NOT IN  (SELECT `promotion_id` FROM `pds_redim_cupons` WHERE `order_id` = '.$order_id.') '.$_rwd_cond.' 
					AND `pds_list_promotions`.`start_date` <= CURDATE() AND `pds_list_promotions`.`end_date` >= CURDATE() 
					ORDER BY `pds_list_promotions`.`id` DESC';
			//echo $_sql;		
...
/*function getAvailPromForOrder($order_id){
	$rows = array();
	if(is_gt_zero_num($order_id)){
		$res = mysql_query('SELECT  pds_list_promotions.id , pds_list_promotions.list_id , pds_list_promotions.title, pds_list_promotions.end_date FROM pds_list_promotions WHERE cupon_type="all_site" AND pds_list_promotions.`prm_restaurent`='.$_SESSION[SES_RESTAURANT].' AND pds_list_promotions.id NOT IN  (SELECT promotion_id FROM `pds_redim_cupons` WHERE order_id = '.$order_id.')  AND pds_list_promotions.start_date <= CURDATE() AND pds_list_promotions.end_date >= CURDATE() ORDER BY pds_list_promotions.id DESC'); 
		if($res){
			while($row = mysql_fetch_assoc($res)){
				$rows[]  = $row;
			}
...
function applyPromotionToOrder($order_id){
 ...
 if(is_gt_zero_num($order_id)){	
	...
	/*$tb_sess_ords=tbl_orders::getOrdGrAmtBySession($cust_sess_id);
	$order_bill_amount=$tb_sess_ords['orders'][$order_id];*/
	$order_bill_amount=$order_detail['gr_amt'];
	$order_items=$order_detail['order_details'];
	//..fetch all claimed promotion for that order	
	$allcopons = $cupons->readArray(array('order_id'=>$order_id,'biz_not_redimed'=>1,SORT_ON=>'apply_type',SORT_BY=>'ASC'),$prom_count);
	...
	if(!$prev_applied && $_is_allowed_based_on_day_time)
	{
		...
		if(is_gt_zero_num($ret_success)){
			...
			//..Update the order_isclaimed_promotions to 0
	 		DB::ExecNonQry("UPDATE `tbl_orders` SET `order_isclaimed_promotions`=1 WHERE `order_id`={$order_id};");
	 		//..break if it is on complete order stop
	 		if($prom_detail['disc_aply_type']=='ORDER'){
	 			break;
	 		}
