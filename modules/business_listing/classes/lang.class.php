...
class lang {
	...
	//
	//Modified mysql query structure for compatibility with phpDirectorySource
    ...
    function lang() {
        ...
        if (!$this->native)
            die("lang() class: unable to determine native language. " .
...
### Private methods:
    ...
    function _query($query) {
		...
		$r_lang = mysql_query($query) or die(mysql_error()."-> ".$query) or die("mysql error in lang() class: ".$query);
		@$lang_rows = mysql_num_rows($r_lang);
		for ($x=0; $x < $lang_rows; $x++){
			$result[] = mysql_fetch_array($r_lang);
		}
		@mysql_free_result($r_lang);
