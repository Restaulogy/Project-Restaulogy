...
if ((isset($_REQUEST['id'])) && (($_REQUEST['id'])>0)){
    $list_id =  getListIdByPromotion($_REQUEST['id']);
...
if((isset($_REQUEST['view_promotion_id'])) && (($_REQUEST['view_promotion_id'])>0)){
    $view_promotion_id = $_REQUEST['view_promotion_id'];
    $is_view_promotion = 1;
}else{
    $view_promotion_id = 0;
    $is_view_promotion  = 0;
...
/// For Deleting Promotion Start ==================================================
 if (( isset($_REQUEST['delete'])) && ($id != 0)) {
	...
	if($is_deleted == 0){
		$info['operation']  ="Problem During Deleting Promotion.";
		$info['is_save_success'] =  0;
 	}elseif($is_deleted == 1){
        $info['operation']  ="Promotion Deleted Succesfully..";
       $info['is_save_success']  =  1;
  	}
...
if ($is_save) {
	...
	$title   = string_replace_for_sql($_REQUEST['title']);
	
	//.. For stripping bad tags from the comment
	require_once("../../../htmlfixer/htmlfixer.class.php");
	$a = new HtmlFixer();
	$PDF_comments = $_REQUEST['comments'];
	$comments = $_REQUEST['comments'];
  
    if((isset($_REQUEST['states']))&&((strlen(trim($_REQUEST['states'])))!=0)){
        $states =  $_REQUEST['states'] ;
    }else{
        $states = 0;
    }
	
	if((isset($_REQUEST['metro_area']))&&((strlen(trim($_REQUEST['metro_area'])))!=0)){
    ...
    if(is_not_empty($_REQUEST['cupon_type'])){
		$cupon_type = $_REQUEST['cupon_type'];
	}
	$allowed_cupons = 0;
    if(is_not_empty($_REQUEST['allowed_cupons'])){
		$allowed_cupons = $_REQUEST['allowed_cupons'];
	}
	...
	if ($id == 0 || is_gt_zero_num($_REQUEST['is_renew_promotion'])){
 	// FOr Insertion
		...
		$info['operation'][]  ='Promotion Created Succesfully';

		if (is_plugin_enabled('CRM')){
				$info['notice'][]  ="<a rel=\"external\" target=\"_blank\" href=\"{$CONFIG->wwwroot}pg/crm/lead/?tbclick=1&type=3\">{$translations['promotion']['publish']['sharewithcrm']}</a>"; 
		...
		$info['new_id'] = $insert_id;
		$info['is_new_insert']  =1; 
        $is_save_success = 1;
    }else{
        $info['op_error']  = 'Not Allowed To Create More Than Two Promotions.';
	}
	}else{
	...
	// for updation
        ...
        if($obj_pds_list_promotions->readObject(array("id"=>$id))){
            ...
            $info['operation'] [] = 'Promotion Updated Succesfully';
            $info['is_new_insert']= 0;
            $is_save_success = 1;
        }
	...
	$error_types = array(
 	...
 	}elseif(((isset($_REQUEST["use_list_img"])) && ($_REQUEST["use_list_img"]==1)) || ((strlen(trim($PDF_IMG_LOGO)))==0)){
  ...
  $is_pdf_uploaded = 0;
            ...
            if ($_FILES[$PDF_NAME]["error"] > 0){
             $is_user_pdf = 0;
    		    ...
    		    if ($pathinfo['extension'] !='pdf') {
    		        $info['op_error']= 'Please Upload Only PDF File';
    		   	}else{
	...
	$info['is_save_success'] = $is_save_success;
