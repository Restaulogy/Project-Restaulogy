{if $smarty.session[$smarty.const.SES_ORDER_UDP]}
	 {assign var=dish_cart value=$smarty.session[$smarty.const.SES_ORDER_UDP][$sequence_num][$smarty.const.SES_SUB_MENU_DISH][$tbl_submenu_dishesinfo.sbmnu_dish_id]} 
{/if}

<div data-role="popup" id="popupNewOrder" data-theme="a" data-dismissible="false" data-theme="a" data-overlay-theme="g" style="width:270px;" class="ui-corner-all">
	...
	<form  id="view_dish_pricing" name="view_dish_pricing"  method="post" action="{$website}/user/add2orderupdate.php">
			   ...
			   {else}
				<td class="bigListItem" style="width:80%;vertical-align: middle !important;"> 
				 ...
				 {/if}
					...
					{for $i=1 to 10}
						<option value="{$i}" {if $dish_cart[$smarty.const.SES_DISH_OPTION_VALUE][{$optItm.dish_opt_val_id}].qty  eq $i}selected="selected"{/if}>{$i}</option>
					{/for}
					</select>
						</div>
...
{include file="tbl_orders/update_js.tpl"}