...
if(strPhone.indexOf("+")>1) { alert('Invalid number format'); return false;}
if(strPhone.indexOf("+")==0) strPhone=strPhone.substring(1)
if(strPhone.indexOf("(")==-1 && strPhone.indexOf(")")!=-1) { alert('Invalid phone number format'); return false;}
if(strPhone.indexOf("(")!=-1 && strPhone.indexOf(")")==-1) { alert('Invalid phone number format'); return false;}
...
if(CCode!="" && CCode!=null){
	if(CCode!="1" && CCode!="011" && CCode!="001") { alert('Invalid phone number format'); return false;}
	}
if(NPA!=""){
  if(checkAreaCode(NPA)==false){ //Checking area code is vaid or not
      alert('Invalid area code in phone number');
      return false;
	}
}
else{return false}
...
function wordwrap( str, width, brk, cut ) {
    ...
    function isUSZip(zipcode,country)
    {
		  //alert('country='+country);		  
		  if (typeof(country)==='undefined'){
		  	 country="{/literal}( $smarty.session.curr_restant.restaurent_country ? $smarty.session.curr_rest.restaurent_country : 'US'){/literal}";	
		  }
		  
		  if(country=='US'){
		  	return /^\d{5}(-\d{4})?$/.test(zipcode);
		  }else{
		  	return isIndianZip(zipcode);
		  }  
    }
...
function isPhoneNumber(str,country){
  ...
  //return re.test(str);
  //..check if we can get from session
  if (typeof(country)==='undefined'){
  	 country="{/literal}( $smarty.session.curr_restant.restaurent_country ? $smarty.session.curr_rest.restaurent_country : 'US'){/literal}";	
  }
  if(country=='US'){
  	...
  	return isIndianMobileNumber(str);
  }  
  return true;
}
