  ...
  {for $i=0;$i<5;$i++}
 	<tr>
		{for $j=0;$j<5;$j++}
         ...
         <table>
            <tr>
                <td width='90%' id="cell_{$gridtable["{$i}_{$j}"].table_id}" posX="{$i}" posY="{$j}" {if $gridtable["{$i}_{$j}"]} style="background-image: url('{$website}/images/tables/{$gridtable["{$i}_{$j}"].table_id}.png');" {/if} onclick="getTblStatus({$gridtable["{$i}_{$j}"].table_id},{$gridtable["{$i}_{$j}"].customer_session});">
                </td>
                <td width='10%' >
