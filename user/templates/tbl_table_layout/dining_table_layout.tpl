...
<style type="text/css"> 
   ...
   table#div2 td span{
        display:inline-block;width:80%;height:80%;background-position: left;
		background-repeat: no-repeat;
...
<div class="wrapper" > 
<h1>{$_lang.tbl_dining_table.layout}</h1>
  ...
  {for $i=0;$i<5;$i++}
 	<tr>
		 ...
		 {if $gridtable["{$i}_{$j}"]}
             ...
             <div style='display:inline-block;width:170px;height:100px;'>
            <!-- <div style='display:inline-block;width:100%;height:80%;'> -->
            <span id="img_{$gridtable["{$i}_{$j}"].table_id}" onclick="getTblStatus({$gridtable["{$i}_{$j}"].table_id},{$gridtable["{$i}_{$j}"].customer_session});" style="background-image: url('{$website}/images/tables/{$gridtable["{$i}_{$j}"].table_id}.png');"></span>
             ...
             <ul style="float:right;">
                <li><img src="{$website}/css/jqm_extra_icon/green/chat.png" onclick="chkNotification({$gridtable["{$i}_{$j}"].table_id},{$gridtable["{$i}_{$j}"].customer_session});" {if $gridtable["{$i}_{$j}"].notifications gt 0}class="active"{/if} title="{$_lang.tbl_alerts.title}"/></li>
             </ul>

         {assign var='tbl_table_status_linkitem' value=$gridtable["{$i}_{$j}"].tbl_status_lnk}
         {if $tbl_table_status_linkitem}
...
{if $active_page eq 'tbl_table_status_link' OR $active_page eq 'table_layout'}
    {literal}
             list_view_lnk="{/literal}{$website}{literal}/user/tbl_table_status_link.php?latestOnly=1&customer_session_id="+ customer_session +"&pst_table_id=" + table_id;
    {/literal}
{elseif $active_page eq 'tbl_orders'}
    {literal}
             list_view_lnk="{/literal}{$website}{literal}/user/tbl_orders.php?order_session_id="+ customer_session + "&table_status=0&table_id="+ table_id;
    {/literal}
{elseif $active_page eq 'employee_requests'}
    {literal}
             list_view_lnk="{/literal}{$website}{literal}/user/employee_requests.php?table_id="+ table_id +"&sess_id="+ customer_session;
    {/literal}
{elseif $active_page eq 'coupon_statistics'}
    {literal}
             list_view_lnk="{/literal}{$website}{literal}/modules/business_listing/coupon_statistics.php?table_id="+ table_id +"&cust_sess_id="+ customer_session +"&table_status=0";
    {/literal}
{elseif $active_page eq 'tbl_alerts'}
    {literal}
             list_view_lnk="{/literal}{$website}{literal}/user/tbl_alerts.php?alert_post_type=&search_isLiveOnly=1&with_notification=1&customer_session_id="+ customer_session +"&search_table=" + table_id;
    {/literal}
{/if}
{literal}
    	//window.location.href="{/literal}{$website}{literal}/user/tbl_table_status_link.php?latestOnly=1&customer_session_id="+ customer_session +"&pst_table_id=" + table_id;
    	window.location.href=list_view_lnk;
    }
    ...
    function chkNotification(table_id,customer_session){
		window.location.href="{/literal}{$website}{literal}/user/tbl_alerts.php?alert_post_type=&search_isLiveOnly=1&with_notification=1&customer_session_id="+ customer_session +"&search_table=" + table_id;
	}
