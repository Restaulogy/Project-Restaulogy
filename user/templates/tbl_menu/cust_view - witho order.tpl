...
{/if}
   ...
   {foreach from=$mnu_full_deatils.submenu item=sub_menu name='_nm_sb_mnu'}
        ...
        <div data-role="collapsible" class="coll" {if $smarty.foreach._nm_sb_mnu.index eq 0}data-collapsed="false"{/if}>
			...
			<table class="listTable white_border no_margin full_width" style='background:#000;'>    
			...
			{/if}
				 	<td >
						<table style='border:0px solid #dadad1;width:100%;height:100%;'>
							<tr>
								<td style='padding:5px;width:100px;height:{if $deviceType eq 'phone'}100px;{else}190px;{/if}background:url("{$dish_img_actual}");background-size:100% 100%;background-repeat: no-repeat;cursor:pointer;' onclick="window.location.href='{$website}/user/tbl_submenu_dishes.php?sbmnu_dish_id={$tbl_dishesitem.sbmnu_dish_id}&is_preview=1&web_redt={$web_redt}';"><span style='float:right;color:orange;font-size:16px;font-weight: bold;font-family: arial;'>{$smarty.session.curr_restant.restaurent_currency}{$dish_price_actual}<br>{if $smarty.session.rest_menu_opt_det.rst_mnu_orders eq 1}							
					<!--	<a href="{$website}/user/tbl_submenu_dishes.php?sbmnu_dish_id={$tbl_dishesitem.sbmnu_dish_id}&is_preview=1&{$smarty.const.POPUP_WINDOW}=NewOrder"  data-inline="true" data-mini="true" data-role="button" data-transition="pop"  data-icon="add-item" data-theme="b">{$_lang.main.order.add_title}</a>-->
													
						{/if}</span></td>
							</tr>
							<tr>
							<!--onclick="window.location.href='{$website}/user/tbl_submenu_dishes.php?sbmnu_dish_id={$tbl_dishesitem.sbmnu_dish_id}&is_preview=1&web_redt={$web_redt}';" -->
								<td style='color:White;font-size:12px;font-family: arial;height:40px;cursor:pointer;' >
									<table>
										<tr>
											<td width='90%'>
											...
											<td width='10%'>
												<!--<input type="button" data-inline="true" data-mini="true" data-icon="add-item" data-theme="b" value="{$_lang.main.order.add_title}" onclick="newOrder({$tbl_dishesitem.sbmnu_dish_id});" />-->
											</td>
										</tr>
									</table>								
								</td>	
