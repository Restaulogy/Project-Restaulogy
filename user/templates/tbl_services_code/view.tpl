...
<div class="navTable_border" style="display:hidden;">
    <table class="navTable" style="width:150px;">
    	<tr>
    	<!--	<th><b id="lbl_inactive_code">Details</b><a href="#" onclick="toggleBlock('code');"  class="biz_hidden" id="lbl_active_code">Details </a></th>
    		<th><b id="lbl_inactive_detail"  class="biz_hidden">Options</b><a  href="#" id="lbl_active_detail" onclick="toggleBlock('detail');">Options</a></th>
    		<th><b id="lbl_inactive_stage"  class="biz_hidden">Stages</b><a  id="lbl_active_stage"  href="#" onclick="toggleBlock('stage');">Stages </a></th>  -->
    	</tr>
    </table>
</div>
...
<div id="tbl_services_code_view" class="description"  style="{if $isUpdate eq 1}display:none;{/if}">
	...
	<tr class="biz_hidden">
	<td class='fieldItem' style="display: none;">{$_lang.tbl_services_code.label.srvc_cat_id}:</td><td class='valueItem'>{$tbl_services_codeinfo.category.srvc_cat_code}</td>
	</tr>
	<tr class="biz_hidden">
...
<center>
{if $tbl_services_codeinfo.isActive eq 1}
	<input type="button" value="{$_lang.tbl_services_requests.DEACTIVATE.BTN_LBL}" onclick='window.location.href="{$website}/user/tbl_services_code.php?action=deactivate&srvc_id={$tbl_services_codeinfo.srvc_id}";' data-inline="true" data-icon="check" title="{$_lang.tbl_services_requests.DEACTIVATE.BTN_LBL}" />
{else}
	<input type="button" value="{$_lang.tbl_services_requests.ACTIVATE.BTN_LBL}" onclick='window.location.href="{$website}/user/tbl_services_code.php?action=activate&srvc_id={$tbl_services_codeinfo.srvc_id}";' data-inline="true" data-icon="check" title="{$_lang.tbl_services_requests.ACTIVATE.BTN_LBL}" />
{/if}
...
{else}	
    ...
    <input type="button" value="{$_lang.service_details.title}" data-icon="search" data-inline="true" onclick="$('#service_detail_block').removeClass('biz_hidden');"/> 
{/if}
...
<!--<div id="service_detail_block" class="biz_hidden">-->
{include file="service_details.tpl"}
<!--</div> -->
 
	
</div> 
<div id="stage_block" class="biz_hidden"> 
	{include file="tbl_services_code/tbl_service_stage.tpl"}
</div> 
</div>
...
<script type="text/javascript">
	function toggleBlock(block_id){
		//.. enable all inactive cell
		/*$('#lbl_inactive_code, #lbl_inactive_detail, #lbl_inactive_stage').addClass('biz_hidden');*/
		$('#lbl_inactive_code, #lbl_inactive_stage').addClass('biz_hidden');
		//.. disable all active cell
		/*$('#lbl_active_code, #lbl_active_detail, #lbl_active_stage').removeClass('biz_hidden');*/
		$('#lbl_active_code, #lbl_active_stage').removeClass('biz_hidden');
		//.. disable all blocks
		/*$('#code_block, #detail_block, #stage_block').addClass('biz_hidden');*/
		$('#code_block, #stage_block').addClass('biz_hidden');
		//..now for activate block
		//..activate block
		$('#' +block_id +'_block').removeClass('biz_hidden');
		//..activate inactive cell
		$('#lbl_inactive_' +block_id +'').removeClass('biz_hidden');
		//..deactivate active cell
		$('#lbl_active_' +block_id +'').addClass('biz_hidden'); 
	}
